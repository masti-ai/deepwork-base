...
if [ -d "$INSTALL_DIR" ]; then
  ...
  cd "$INSTALL_DIR" && git pull --quiet origin main && cd - >/dev/null
else
...
# Step 4: Make mesh scripts executable
echo ""
echo "[4/5] Setting up mesh commands..."
chmod +x "$INSTALL_DIR/scripts/"*.sh 2>/dev/null || true

# Create symlink so `gt mesh` works (if gt supports plugins)
if [ -d "$GT_ROOT/mayor" ]; then
  # Copy mesh dispatcher to a location gt can find
